import argparse
from helpers import run_command, run_script
...
def get_base_latency():
    ...
    result = run_command(["ping", "-c", str(10), my_location.get("ip")])
    if result is None:
        return "0", my_location.get("loc").split(",")
    
    # Filter output for lines containing "time="
    filtered_output = [line for line in result.splitlines() if "time=" in line]
    times = [float(line.split(" ")[-2].replace("time=", "")) for line in filtered_output] 
    
    return str(min(times)), my_location.get("loc").split(",")

def run_scripts(domain, base_latency, geopoint, fetch_websites, agg_func, vpn):
    #if fetch_websites:
    #    run_script(["python3", "fetch_websites.py", args.country, args.code, "country", args.semester, args.count, "filter_dns"])
    
    #run_script(["python3", "ping.py", domain, base_latency])
    #run_script(["python3", "ping_ipv6.py", domain, base_latency])
    run_script(["python3", "findcdn.py", domain, vpn])
    run_script(["python3", "geolocate.py", domain, vpn, agg_func])
    run_script(["node", "locedge/locedge.js", domain, args.country, vpn]) 
